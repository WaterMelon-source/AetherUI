...
-- // Theme Configuration (FIXED Font API)
local THEME = {
	Window = {
	...
	Font = Font.fromEnum(Enum.Font.Gotham),        -- ✅ Fixed
	FontBold = Font.fromEnum(Enum.Font.GothamBold), -- ✅ Fixed
}

-- // Utility Functions (FIXED nil props)
...
function Window.new(title: string, description: string?, options: table?): table
	...
	local screenGui = createInstance("ScreenGui", PlayerGui, {
		Name = "AetherUI", ZIndexBehavior = Enum.ZIndexBehavior.Sibling,
	})
	local windowFrame = createInstance("Frame", screenGui, {
		Name = "Window", ZIndex = 10, BorderSizePixel = 0,
		BackgroundColor3 = THEME.Window.Background, AnchorPoint = Vector2.new(0.5, 0.5),
	...
	local side = createInstance("Frame", windowFrame, {
		...
		BackgroundColor3 = THEME.Window.SideBackground, AnchorPoint = Vector2.new(0.5, 0.5),
	...
	createInstance("UIStroke", side, {
		Color = THEME.Window.StrokeColor, ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
	})
	
	local decor = createInstance("Frame", side, {
		Name = "Decor", BackgroundTransparency = 1, AnchorPoint = Vector2.new(0.5, 0.5),
	...
	for i, color in ipairs(colors) do
		local dot = createInstance("Frame", decor, {
			BorderSizePixel = 0, BackgroundColor3 = color, AnchorPoint = Vector2.new(0.5, 0.5),
			Size = UDim2.new(1, 0, 1, 0), Position = UDim2.new((i-1)*0.5, 0, 0.5, 0),
		})
	...
	local tabsContainer = createInstance("Frame", side, {
		...
		BackgroundColor3 = Color3.fromRGB(26, 26, 26), AnchorPoint = Vector2.new(0.5, 0.5),
	...
	local main = createInstance("Frame", windowFrame, {
		...
		BackgroundColor3 = Color3.fromRGB(187, 187, 187), AnchorPoint = Vector2.new(0.5, 0.5),
	...
	local labelsHeader = createInstance("Frame", main, {
		Name = "Labels", BackgroundTransparency = 1, AnchorPoint = Vector2.new(0.5, 0.5),
	...
	local scrollingFrame = createInstance("ScrollingFrame", main, {
		...
		ScrollBarImageColor3 = Color3.fromRGB(61, 61, 61), ScrollBarThickness = 4,
		BackgroundTransparency = 1,
	})
...
function Window:AddTab(name: string, icon: string?): table
	local tabBtn = createInstance("Frame", self._tabsContainer, {
		Name = "Tab", BorderSizePixel = 0, BackgroundColor3 = Color3.fromRGB(56, 56, 56),
	...
	local button = createInstance("TextButton", tabBtn, {
		...
		TextSize = 12, TextColor3 = THEME.Window.TextPrimary,
		BackgroundColor3 = Color3.fromRGB(56, 56, 56), FontFace = THEME.FontBold,
...
function Library:SetTheme(newTheme: table)
	for key, value in pairs(newTheme or {}) do
		if type(value) == "table" and THEME[key] then
			for k, v in pairs(value) do THEME[key][k] = v end
